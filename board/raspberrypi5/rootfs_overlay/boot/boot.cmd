...
# ─── TPM 2.0 startup ─────────────────────────────────────────────────────────
# TF-A/OP-TEE may have already called TPM2_CC_Startup; we ignore the error.
...
# RAUC sets BOOT_ORDER ("A B" normal, "B A" after update) and BOOT_x_LEFT
# (retry counters).  U-Boot decrements the counter each boot and saves it.
# rauc-mark-good.service resets BOOT_x_LEFT on a confirmed successful boot.
...
# ─── Boot arguments ───────────────────────────────────────────────────────────
setenv bootargs "console=ttyAMA10,115200 console=tty1 rootfstype=ext4 rootwait ro quiet loglevel=3 panic=5 ima_policy=tcb ima_appraise=enforce apparmor=1 security=apparmor systemd.unified_cgroup_hierarchy=1 cgroup_memory=1 cgroup_enable=memory root=/dev/mmcblk0p${rootpart} rauc.slot=${rauc_slot}"
